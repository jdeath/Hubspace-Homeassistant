...
def get_refresh_code(user_name, password):
    ...
    headers = {}
    r = requests.post(auth_url, data=auth_data, headers=auth_header, cookies=r.cookies.get_dict(),
...
def get_child_id(refresh_token, account_id, device_name, only_print_anonymized_json=False):
    token = get_auth_token_from_refresh_token(refresh_token)
...
def get_conclave(refresh_token, account_id):
    ...
    print(port)
    header = {
        "login": {"channelId": "XXXXX", "accessToken": token, "type": "android", "version": "1.0.0", "protocol": 2,
                  "trace": True}}
    return token
...
passwd = getpass.getpass(prompt='Hubspace Password: ', stream=None) if args.password is None else args.password

obtained_refresh_token = get_refresh_code(user, passwd)

obtained_account_id = get_account_id(obtained_refresh_token)

get_child_id(obtained_refresh_token, obtained_account_id, None, only_print_anonymized_json=True)
